...
void Hash_set<T>::insert(T key) {
  ...
  for (int i = 0, j = get_index(key_hash, i); i < m;
    ...
    if (hash_table[j % m] == nullptr || hash_table[j % m]->get_is_deleted()) {
      auto *new_node = new Node_set<T>(key, j);
      hash_table[j % m] = new_node;
      hash_table[j % m]->set_is_deleted(false);
      len++;
      if (len * 2 >= m) {
        vector<Node_set<T> *> new_hash_table(2 * m, nullptr);
        for (int k = 0; k < m; k++) {
          if (hash_table[k] == nullptr)
            continue;
          for (int i_ = 0, j_ = get_index(hash_table[k]->get_key_hash(), i_); i_ < 2 * m;
               j_ = get_index(hash_table[k]->get_key_hash(), ++i_)) {
            if (new_hash_table[j_ % (2 * m)] == nullptr || new_hash_table[j_ % (2 * m)]->get_is_deleted()) {
              new_hash_table[j_ % (2 * m)] = hash_table[k];
              break;
            }
            if (new_hash_table[j_ % (2 * m)]->compare(hash_table[k]->get_key(), hash_table[k]->get_key_hash())
                && !new_hash_table[j_ % (2 * m)]->get_is_deleted())
              break;
          }
        }
...
void Hash_set<T>::erase(T key) {
  ...
  for (int i = 0, j = get_index(key_hash, i); i < m;
    ...
    if (hash_table[j % m]->compare(key, key_hash) && !hash_table[j % m]->get_is_deleted()) {
      hash_table[j % m]->set_is_deleted(true);
      len--;
      return;
    }
  }
