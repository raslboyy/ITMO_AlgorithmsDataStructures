...
class Node {
 public:
  Node(int key) : key(key), diff(0), l(nullptr), r(nullptr) {};
  int key;
  int diff;
  int num;
  Node *l;
...
Node *AVL::rotate(Node *a) {
  int d = a->diff;
  if (d == -2) {
//    if (a->r == nullptr || (a->r->r == nullptr && a->r->l == nullptr)) {
//      cout << "error" << endl;
//      exit(-1);
//    }
    Node *b = a->r;
...
//    if (a->l == nullptr || (a->l->r == nullptr && a->l->l == nullptr)) {
//      cout << "error" << endl;
//      exit(-1);
//    }
    Node *b = a->l;
...
void AVL::print(Node *node) {
  if (node == nullptr)
    return;
  cout << node->key << " " << (node->l == nullptr ? 0 : node->l->num) << " " \
 << (node->r == nullptr ? 0 : node->r->num) << '\n';
  print(node->l);
  print(node->r);
}
...
//  freopen("output.txt", "w", stdout);
  ...
  while (t--) {
    ...
    if (command == 'A')
      ...
      cout << -tree.erase(value) << endl;
    else
      cout << (tree.exist(value) ? 'Y' : 'N') << endl;
  }
